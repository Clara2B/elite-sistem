...
| 4 — Autenticação, Multiempresa, Setores e Permissões | 🟡 Implementada, aguardando deploy/validação em produção | Login individual, papéis (Admin Superior, T.I., Líder, Colaborador), setores reais da Clara, segregação por operadora aplicada em todas as rotas, auditoria. 37 testes automatizados. Falta a Clara configurar o bootstrap no Render e confirmar em produção. Ver `DATABASE.md` seção 8, `SECURITY.md` seção 5, `DECISIONS.md`. |
| 5 — Gestão de Processos (ELITE) | ⬜ Não iniciada | Requer levantamento de requisitos dedicado — setor "Líder - Gestão de Processos" já existe no modelo, esperando o módulo. |
