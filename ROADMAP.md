...
| 5 — Gestão de Processos (ELITE) | 🟢 Concluída (validação local) | Import, relatório (individual+geral, texto e PDF com identidade ELITE) e "prazos próximos" (alerta só dentro do sistema — e-mail removido a pedido da Clara) implementados. Import validado contra a planilha real: 44.333 eventos, 5.636 processos, 6.303 prazos fatais. `prazo_fatal` corrigido para só marcar quando a coluna é "SIM". 50 testes automatizados. `data_prazo` só existe para lançamentos daqui pra frente (histórico não tem data estruturada). Botão "Authorize" do `/docs` corrigido (não aparecia em nenhuma fase). Falta: validar o import via API em produção. Ver `ARCHITECTURE.md` seção 3.5 e `DATABASE.md` seção 6.1/8. |
